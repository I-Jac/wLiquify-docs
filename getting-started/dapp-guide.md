...
Currently, bridging assets to Solana requires a manual process:

1. **Bridge to Source Chain**
   - Use a bridge to transfer your assets to the source chain
   - Ensure you have enough gas for transactions on the source chain

2. **Buy Source Token**
   - On the source chain, buy the token you want to bridge to Solana
   - Make sure to buy the correct token that has a Wormhole NTT wrapped version on Solana

3. **Bridge to Solana**
   - Use Wormhole NTT to bridge the token to Solana
   - You'll receive the Wormhole-wrapped version of the token on Solana
   - This wrapped token must be whitelisted in the wLiquify pool

4. **Future Automation**
   - We plan to automate this process in the future
   - Users will be able to complete the entire process through our frontend
   - Reduced manual steps and better user experience
